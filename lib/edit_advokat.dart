...
class EditAdvokat extends StatefulWidget {
  final String docId;

  const EditAdvokat({Key? key, required this.docId}) : super(key: key);

  @override
  _EditAdvokatState createState() => _EditAdvokatState();
}

class _EditAdvokatState extends State<EditAdvokat> {
  File? _imageFile;
  String? _imageUrl;
  ...
  void initState() {
    super.initState();
    _fetchAdvokatData(widget.docId);
  }

  Future<void> _fetchAdvokatData(String docId) async {
    DocumentSnapshot doc = await FirebaseFirestore.instance.collection('advokat').doc(docId).get();
    if (doc.exists) {
      setState(() {
  ...
  Future<void> _uploadImage(String docId) async {
    if (_imageFile != null) {
      try {
        final ref = FirebaseStorage.instance.ref().child('advokat_images').child('$docId.jpg');
        await ref.putFile(_imageFile!);
        String imageUrl = await ref.getDownloadURL();
        await FirebaseFirestore.instance.collection('advokat').doc(docId).update({
          'image': imageUrl,
        });
        setState(() {
          _imageUrl = imageUrl;
  ...
  Future<void> _saveChanges() async {
    try {
      ...
      await FirebaseFirestore.instance.collection('advokat').doc(widget.docId).update({
